...
struct DemoSnapshotGenerator {
    ...
    private func memoryPressure(forUsagePercent pct: Double) -> Double {
        let p: Double = switch pct {
        case ..<50: pct * 0.2
        case ..<70: 10 + (pct - 50) * 1.0
        case ..<85: 30 + (pct - 70) * 2.0
        case ..<95: 60 + (pct - 85) * 3.0
        default: 90 + (pct - 95) * 2.0
        }
    ...
    private func demoProcesses(cpu: Double, usedGB _: Double) -> [ProcessSample] {
        [
            ProcessSample(pid: 412, name: "swift-frontend", cpuPercent: (cpu * 1.4).clamped(to: 5 ... 780), memoryMB: 2300),
            ProcessSample(pid: 88, name: "Xcode", cpuPercent: (cpu * 0.5).clamped(to: 2 ... 160), memoryMB: 4100),
            ProcessSample(pid: 901, name: "node", cpuPercent: Double.random(in: 5 ... 60), memoryMB: 1250),
