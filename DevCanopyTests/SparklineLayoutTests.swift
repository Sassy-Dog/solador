...
final class SparklineLayoutTests: XCTestCase {
    ...
    func testFullBufferSpansFullWidth() {
        ...
        XCTAssertEqual(pts.last!.x, 119, accuracy: 0.01)
    }

    /// More values than capacity keeps only the most recent `capacity`.
    func testOverflowKeepsMostRecent() {
        ...
        XCTAssertEqual(pts.last!.x, 119, accuracy: 0.01)  // newest still pinned right
    }
