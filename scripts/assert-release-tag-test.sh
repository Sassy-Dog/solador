...
# job (Linux, bash 5) and its `rust-workspace` job (macOS, stock /bin/bash
# 3.2 — the interpreter a macOS release runner and `./dev publish` execute
# under), by `./dev test`, and by hand:
#
...
mint() {
    ...
    ( cd "$repo" && VERSION_DATE_OVERRIDE="$today" "$BASH" "$MINT" --tag --push 2>"$work/mint.err" ) \
        | sed -n 's/^tag=//p'
}

# bare_version REPO TODAY — the wall-clock derivation the old assertion used.
bare_version() {
    ( cd "$1" && VERSION_DATE_OVERRIDE="$2" "$BASH" "$MINT" --version )
}
...
expect_mentions "ended before this commit was made \(2026-09\)" "     ...naming the commit's month"
...
# --- shape ---------------------------------------------------------------------
expect refuse "a semver tag" "$repo" "v1.2.3"
expect refuse "a padded month" "$repo" "v2026.08.5"
expect refuse "a padded patch" "$repo" "v2026.8.05"
expect refuse "a .0 patch (the floor never emits one)" "$repo" "v2026.8.0"
expect refuse "a thirteenth month" "$repo" "v2026.13.1"
expect refuse "a year with a leading zero" "$repo" "v0999.1.1"
expect refuse "a suffix" "$repo" "v2026.8.5-rc1"
expect refuse "no v prefix" "$repo" "2026.8.5"
expect refuse "not a tag at all" "$repo" "release"
