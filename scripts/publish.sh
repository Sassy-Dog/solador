...
if [[ -n "$TAG" ]]; then
    ...
    log_info "     pre-releases — and, behind ONE MORE 'prd' approval, the agent's"
    log_info "     signed agent-latest.json (#391). Windows has no in-app updater:"
    log_info "     its users download the next installer."
    log_info ""
    ...
    log_info "publish-feed.yml's desktop feed job run 'Assert the tag matches the"
    log_info "derived CalVer' at the tag. So after the month rolls, a re-run of"
    log_info "release.yml for $TAG fails (observed on v2026.8.139, attempt 2), and"
    log_info "publishing a still-unpublished draft fires a feed run whose desktop"
    log_info "half fails, leaving releases/latest with no latest.json for every"
    log_info "installed cockpit to 404 on (the agent feed asserts the approved"
    log_info "commit rather than the date, and still lands). The remedy for both"
