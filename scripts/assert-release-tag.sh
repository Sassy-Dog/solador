...
commit_month="$(TZ=UTC git log -1 --date=format-local:%Y%m --format=%cd "$head" 2>/dev/null || true)"
if [[ ! "$commit_month" =~ ^[0-9]{6}$ ]]; then
    refuse "git could not report HEAD's committer date, so the pinned-month derivation could not run"
fi
if (( tag_month < 10#$commit_month )); then
    refuse "its month $year.$month ended before this commit was made (${commit_month:0:4}-${commit_month:4:2}), which the mint never does (a hand-made tag, or a tag on the wrong commit)"
fi
...
if ! reported="$(env -u MARKETING_VERSION -u VERSION_PATCH_OVERRIDE \
        VERSION_DATE_OVERRIDE="$pin" bash "$MINT" --tag)"; then
